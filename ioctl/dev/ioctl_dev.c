...
static int ioctltest_open(struct inode *inode, struct file *filp)
{
	return 0;
}

static int ioctltest_release(struct inode *inode, struct file *filp)
{
	return 0;
}

static long ioctltest_unlocked_ioctl(struct file *filp, unsigned int cmd,
		unsigned long arg)
{
	...
	if (_IOC_TYPE(cmd) >= IOCTLTEST_MAXNR)
		return -EINVAL;
	...
	case IOCTLTEST_READ:
		...
		ctrl_info.size = 1;
		err = copy_to_user((void *)arg, (const void *)&ctrl_info,
				size);
		break;
	case IOCTLTEST_WRITE:
		err = copy_from_user((void *)&ctrl_info, (const void *)arg,
				size);
...
static struct file_operations ioctltest_fops = {
	.owner = THIS_MODULE,
	.unlocked_ioctl = ioctltest_unlocked_ioctl,
	.open = ioctltest_open,
	.release = ioctltest_release,
};

static int ioctltest_init(void)
{
	int result;

	result = register_chrdev(IOCTLTEST_DEV_MAJOR, IOCTLTEST_DEV_NAME,
			&ioctltest_fops);
	if (result <0) {
